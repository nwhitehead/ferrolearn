//! Statistical equivalence tests comparing ferrolearn KNN models against
...
#[test]
fn sklearn_equiv_kneighbors_classifier() {
    let fixture: serde_json::Value = serde_json::from_str(include_str!(
        "../../tests/fixtures/sklearn_reference/kneighbors_classifier.json"
    ))
    .unwrap();
    ...
    // KNN is deterministic -- predictions should match sklearn closely.
...
#[test]
fn sklearn_equiv_kneighbors_regressor() {
    let fixture: serde_json::Value = serde_json::from_str(include_str!(
        "../../tests/fixtures/sklearn_reference/kneighbors_regressor.json"
    ))
    .unwrap();

    let x = json_to_array2(&fixture["X_train"]);
    let y = json_to_array1_f64(&fixture["y_train"]);
    let expected_preds = json_to_array1_f64(&fixture["predictions"]);
    ...
    assert!(
        max_diff < 1.0,
        "KNeighborsRegressor max prediction diff {max_diff:.6} >= 1.0 vs sklearn"
    );

    // R2 should be close to sklearn's.
    ...
    assert!(
        ...
        "KNeighborsRegressor R2 {r2:.4} too far from sklearn {sklearn_r2:.4}"
    );
}
